...
const req = (url, parseData, resultsField = undefined, rawData = false) => {
	return httpClient
		.get(url, { headers: { 'User-Agent': 'WikiTaxa' } })
		.then(response => {
...
const getTropicos = q => {
	return req(
		`http://legacy.tropicos.org/Ajax/NameFastLookup.aspx?type=NameFastLookupEX&text=${q}&returnCount=${MAX_RESULTS}`,
		data => {
			return data.map(sp => ({
				id: sp.nameID,
				name: sp.displayName
...
const getWoRMS = q => {
	return req(
		`https://www.marinespecies.org/aphia.php?p=rest&__route__/AjaxAphiaRecordsByNamePart/${q}&rank_min=10&combine_vernaculars=0&fossil_id=4&value_raw=${q}`,
		data => {
			return data.map(sp => ({
				id: sp.id,
				name: sp.displayname,
